...
void update(ll start, ll end, ll u, ll v, ll idx)
{
	if(start==end)
	{
		if(start==u)
			tree[idx].sum=tree[idx].msum=tree[idx].pre=tree[idx].suf=arr[u]=v;
		return;
	}
	if(start>end || start>u || end<u)
		return;
	ll mid=(start+end)/2;
	update(start,mid,u,v,2*idx+1);
	update(mid+1,end,u,v,2*idx+2);
