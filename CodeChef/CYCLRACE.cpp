/*
Implementation of Convex Hull Trick is copied from 
...
#define MAX 50000+5
ll dist[MAX]={0},speed[MAX]={0},time[MAX]={0};
/*
...
int main()
{
	ConvexHullDynamic cyclrace(1);
	ll i,j,k,n,q,p,t,cyc,new_speed;
	sc(n); sc(q);
	while(q--)
	{
		sc(p);
		if(p==1)
		{
			sc(t); sc(cyc); sc(new_speed);
			dist[cyc]+=(t-time[cyc])*speed[cyc];
			speed[cyc]=new_speed;
			time[cyc]=t;
			race.addLine(speed[cyc],dist[cyc]-(speed[cyc]*time[cyc]));
		}
		if(p==2)
		{
			sc(t);
