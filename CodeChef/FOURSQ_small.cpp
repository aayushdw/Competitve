...
int p,base[MAXSZ];
vector< pair < ii,ii > > tree(4*MAXSZ);
...
pair< ii,ii > get(pair< ii,ii > x , pair< ii,ii > y)
{
	int x1=x.F.F , x2=x.F.S , x3 = x.S.F , x4 = x.S.S;
	int y1=y.F.F , y2=y.F.S , y3 = y.S.F , y4 = y.S.S;
...
pair< ii,ii > query(int idx,int start,int end,int qs,int qe)
{
	if(start>end || qs>qe || start>qe || end<qs)
		return mp(mp(0,0),mp(0,1));
	if(start>=qs && end<=qe)
		return tree[idx];
	int mid=(start+end)>>1;
